...
//	Copyright (C) S閎astien Kirche (sebastien.kirche@free.fr) 2014
//
...
//	Sourcecode welcher TreeListWnd verwendet muss ver鰂fendlicht werden.
//	Komerzielle Nutzer m黶sen ihren Code ebenfalls ver鰂fentlichen, oder
...
typedef struct {
	...
	unsigned	uNextItem; 									// Ist die Nummer des n鋍hsten Eintrages (0=keines)
	...
	WORD		uTextSize;									// L鋘ge des Textes in Zeichen
	BYTE		bCallback;									// Sind Bits f黵 Callbacks
...
typedef struct {
	...
	LPVOID		hTheme;										// Handle f黵 benutztes Thema (TREELIST)
	LPVOID		hThemeBt;									// Handle f黵 benutztes Thema (BUTTON)
	WNDPROC		pProcId3;									// Fenster Funktion f黵 ID3 Fenster
	HIMAGELIST	hStates;									// Handle der Icon-Liste f黵 States und Overlay
	HIMAGELIST	hImages;									// Handle der Icon-Liste
	HIMAGELIST	hChecks;									// Handle der Icon-Liste f黵 die Checkboxen in den Spalten
	HIMAGELIST	hSubImg;									// Handle der Icon-Liste f黵 die Spalten
	...
	COLORREF	uColors[MAX_COLORS];						// 0=Hintergrundfarbe  1=Abwechselnte Farbe  2=Farbe f黵 Trennlinien  3=Textfarbe
	int			iFontHeight;								// Ist die H鰄e des Fonts
	...
	int			iStatesMode;								// Die hStates Image-Liste wurde f黵 die Checkboxen erzeugt
	int			iStatesXsize;								// Breite der States und Overlay Icons
	int			iStatesYsize;								// H鰄e   der States und Overlay Icons
	int			iChecksMode;								// Die hChecks Image-Liste wurde f黵 die Checkboxen erzeugt
	int			iChecksXsize;								// Breite der States und Overlay Icons
	int			iChecksYsize;								// H鰄e   der States und Overlay Icons
	int			iImagesXsize;								// Breite der Icons
	int			iImagesYsize;								// H鰄e   der Icons
	int			iSubImgMode;								// Die SubImg Image-Liste ist nicht die hImages Liste
	int			iSubImgXsize;								// Breite der Icons
	int			iSubImgYsize;								// H鰄e   der Icons
	int			iRowHeight;									// Ist die H鰄e einer Zeile
	...
	int			iIndent;									// Einr點kung der Kindereint鋑e
	int			iShift;										// Einr點kung der vertikalen Linien
	int			iAutoAdd;									// Offset zum Open-Icon  f黵 TVS_EX_AUTOEXPANDICON
	int			iMaxSizeX;									// Die Gr鲞e des breitesten sichtbaren Eintrages
	unsigned	uItemPosCount;								// Anzahl der sichtbaren Eintr鋑e
	unsigned   *pItemPos;									// Liste mit den Offsets der sichtbaren Eintr鋑e
	BaseItem  **pTreeItems;									// Zeiger auf Item Zeiger
	ExtraItem **pExtraItems[MAX_COLUMNS - 1];				// Zeiger auf die Spalteneintr鋑e
	unsigned	uTreeItemsMax;								// Gr鲞e der Liste mit den vorhanden Eintr鋑e (alociert um 1 gr鲞er)
	unsigned	uTreeItemsCount;							// Anzahl der vorhanden Eintr鋑e
	unsigned	uNextSeachPos;								// N鋍hste Position zum suchen von freien Eintr鋑en
	unsigned	uUserDataSize;								// Ist die Gr鲞e der Userdaten in einem Eintrag
	unsigned	uFirstChild;								// Ist die Nummer des ersten Kind-Eintrages (0=keines)
	unsigned	uLastChild;									// Ist die Nummer des letzten Kind-Eintrages (0=keines)
	unsigned	uSingleSel;									// Ist die Nummer des gew鋒lten Eintrages (bei Checkboxen)
	unsigned	uScrollX;									// Aktuelle X-Scroll-Position
	unsigned	uScrollY;									// Aktuelle Y-Scroll-Position
	unsigned	uSizeX;										// Aktuelle X-Fenster-Gr鲞e
	unsigned	uSizeY;										// Aktuelle Y-Fenster-Gr鲞e
	unsigned	uSizeYsub;									// Aktuelle Y-Fenster-Gr鲞e ohne Header
	...
	unsigned	uMaxEnties;									// Anzahl der sichtbaren Eintr鋑e (inkl. halbsichtbare)
	unsigned	uPageEnties;								// Anzahl der sichtbaren Eintr鋑e (ohne  halbsichtbare)
	unsigned	uColumnCount;								// Anzahl der Spalten
	unsigned	uColumnCountVar;							// Anzahl der variabeln Spalten
	unsigned	uSelectedCount;								// Anzahl der ausgew鋒lten Eintr鋑e
	unsigned	uSelectedBase;								// Ist der Eintrag ab dem gew鋒lt wurde
	unsigned	uSelectedItem;								// Ist der Eintrag der gerade gew鋒lt ist
	unsigned	uSelectedSub;								// Ist die Spalte die gerade gew鋒lt ist
	...
	unsigned	uToolTipItem;								// Ist der ToolTip-Eintrag der gerade gew鋒lt ist
	unsigned	uToolTipShow;								// Ist die Zeitverz鰃erung in 500 ms Schritten f黵 das Tooltip
	unsigned	uToolTipSub;								// Ist die ToolTip-Spalte die gerade gew鋒lt ist
	...
	unsigned	uOldXPage;									// Alte Werte f黵 X-Scroll-Bar
	unsigned	uOldXCount;									//			*
	unsigned	uOldYPage;									// Alte Werte f黵 Y-Scroll-Bar
	unsigned	uOldYCount;									//			*
	unsigned	uTrippleB;									// Bereite des "..." Strings f黵 den fetten Fonts
	unsigned	uTrippleN;									// Bereite des "..." Strings f黵 den normalen Fonts
	...
	unsigned	uInsertMark;								// Ist der Eintrag mit der Einf黦emarke
	unsigned	uMarkedCols;								// Anzahl der markierten Spalten
	unsigned	uDragFlags;									// Welche Maustasten sind an
	unsigned	uDragItem;									// Eintrag f黵 Dragoperation
	unsigned	uDragSub;									// Untereintrag f黵 Dragoperation
	unsigned	uLastSel;									// Letzte Textauswahl beim Editieren
	unsigned	uLastMove;									// Letzte Cursorposition bei WM_MOUSEMOVE
	unsigned 	uButtonPos;									// Wo wurde eine Maustaste wurde zuletzt gedr點kt
	unsigned 	uButtonLast;								// Wann wurde eine Maustaste wurde zuletzt gedr點kt
	unsigned	uToolTipSize;								// Textspeichergr鲞e f黵 Tooltip
	LPTSTR		pToolTipText;								// Textspeicher f黵 Tooltip
	TCHAR		cTempText1   [260];							// Erster Textpuffer f黵 Callbacks
	TCHAR		cTempText2   [260];							// Zeiter Textpuffer f黵 Callbacks
	...
	char		cColorChanged[MAX_COLORS ];					// Welche Farbe wurden ver鋘dert
	char   	    cColumnStart;								// Wurde das Autoeditiren mit einer WM_CHAR Eingabe gestartet
	char 		cFixedHeight;								// Ist eine fixe H鰄e eingestellt
	char		cLockChanges;								// Sperren von Fenster鋘derungen
	char		cHasRootRow;								// Wird gesetzt wenn eine Root-Spalte eingef黦t wird
	char		cKeyIgnore;									// Die n鋍hste Taste nicht f黵 Sucher verwenden
	char		cClickFlag;									// Merker f黵 LBUTTON-DOWN bei Multiselect
	char		cClickEdit;									// Merker f黵 LBUTTON-DOWN bei Edit-Click
	...
	char		cGlyphOk;									// Die Schaltf鋍he 黚er Themen zeichnen
	char		cEditCb;									// Muss das Edit-Fenster einen Callback aufrufen
	char		cButtonFlag;								// Welche Maustaste wurde zuletzt gedr點kt
} TreeListData;
...
//	Ergibt den R點kgabewert der WM_NOTIFY Nachrich
...
//*
//*****************************************************************************
//	Ist die Fensterfunktion f黵 das Edit Fenster
...
//*
//*****************************************************************************
//	Ist die Fensterfunktion f黵 das ToolTip Fenster
...
//*
//*****************************************************************************
//	膎dert die Gr鲞e der variablen Spalten
//	pData		: Zeiger auf die Fensterdaten
//	iDelta		: Ist die Gr鲞en鋘derung in Pixel
static void ChangeColSize(TreeListData *pData, int iDelta) {
	...
	if(iCnt <= 1) {											// Nur eine variable Spalte
		for(uPos = 0; uPos < pData->uColumnCount; uPos++) {
			...
			if(pData->aColumn[uPos].sReal != sItem.cxy) {	// 膎dert sich die Breite
...
static void CreateStateImageList(TreeListData *pData, int iMode) {
	...
	if(pOpenThemeData) {									// 躡er Thema zeichnen
		if(!pData->hThemeBt) {
...
//	Erzeugt eine Image-Liste mit zwei Checkboxen
//	pData		: Zeiger auf die Fensterdaten
//	uSub		: Ist die Spalte f黵 die das Drag-Image erzeugt werden soll
...
static HIMAGELIST CreateDragImage(TreeListData *pData, unsigned uItem, unsigned uSub) {
	...
	if(uSub) {												// Image f黵 Extraeintrag erzeugen
	...
	} else {													// Image f黵 Haupteintrag erzeugen
		pText  = pEntry->pText;
	...
	if(pData->hImages && iImage >= 0) {							// Gr鲞en f黵 Images anpassen
...
//*
//*****************************************************************************
//	Zeichnet einen Eintrag 黚er die ganze Zeile neu
//	pData		: Zeiger auf die Fensterdaten
//	uItem		: Ist die Nummer des Eintrages
//	Ergibt 1 wenn der Eintrag sichtbar war
...
//	pData		: Zeiger auf die Fensterdaten
//	Ergibt 1 wenn der sich Einstellungen ver鋘dert haben
...
//*
//*****************************************************************************
//	Aktualisiert den Text f黵 das Tootip
	...
	if(uFlags & TVHT_ONITEM) {
		if(pData->uToolTipItem != uItem || pData->uToolTipSub != 0) {
			...
			if(sRect.right - sRect.left <= pEntry->iTextPixels + 4) {
				...
				if(uLen >= pData->uToolTipSize) {				// Tooltipspeicher vergr鲞ern
...
//*
//*****************************************************************************
//	Erzeugt einen den fetten Font f黵 das Fenster
//	pData		: Zeiger auf die Fensterdaten
//	iRedraw		: Soll das Fenster neugezeichnet werden
//	Ergibt 1 wenn der Font ver鋘dert wurde
...
*/
	...
	if(pData->hFontN != pData->hFontL) {
		...
		for(; iPos >= 0; iPos--) {								// Alle Textbreiten zur點ksetzen
			pEntry = pList[iPos];
			if(!pEntry)
				continue;
...
//*
//*****************************************************************************
//	Berechnet die Positionen der Zeilen f黵 die sichtbaren Eintr鋑e
//	pData		: Zeiger auf die Fensterdaten
...
static void UpdateItems(TreeListData *pData, unsigned uItem) {
	...
	if(!uItem) {												// Am Anfang beginnen
		...
		if(!uItem) {											// Leere Liste
			...
			for(uNum = 0; uNum < uOld; uNum++) {					// Die alten Eintr鋑e zur點ksetzen
				uTemp = pLines[uNum];
				if(!uTemp)
					continue;
				pLines[uNum] = 0;
		...
		for(uNum = uPos; uNum < uOld; uNum++) {						// Die alten Eintr鋑e zur點ksetzen
			uTemp = pLines[uNum];
			if(!uTemp)
				continue;
			pLines[uNum] = 0;
...
//*
//*****************************************************************************
//	Pr黤t ob es Ver鋘derungen in Spaltenbreiten gab
//	pData		: Zeiger auf die Fensterdaten
//	Ergibt die Breite ab der die Spalten ver鋘dert wurden oder 0x10000
static int UpdateColumns(TreeListData *pData) {
	...
	for(uCol = 0; uCol < pData->uColumnCount;) {					// Suche die erste ge鋘derte Spalte
		...
		if(iNum >= iNow) {
			...
			if(pData->uSelectedItem) {						// Problem bei ausgew鋒lten leeren Eintr鋑en
...
//	pArray		: Zeiger auf die Eintr鋑e. Null steht f黵 die Standartreihenfolge.
...
static int TreeListSetOrderArray(TreeListData *pData, unsigned uItems, unsigned *pArray) {
	...
	for(uCol = 0, uDiff = 0; uCol < uItems; uCol++) {					// Die Eintr鋑e pr黤en
		uSub	= pArray[uCol];
		if(uSub >= uItems)
			return 0;
		if(aFlags[uSub])
	...
	for(uCol = 1; uCol < uItems; uCol++) {							// Die Eintr鋑e anpassen
	...
	if(pData->uStyleEx & TVS_EX_HEADERCHGNOTIFY) {				// Alle Spalten haben sich ver鋘dert
		UNLOCK(pData);
...
//	pData		: Zeiger auf die Fensterdaten
//	uItem		: Ist die Nummer des Eintrages
//	uAddFlags	: Sind die State-Flags die hinzugef黦t werden sollen
...
static int TreeListToggleItem(TreeListData *pData, unsigned uItem, unsigned uAddFlags) {
	...
	if(pEntry == 0)
		return -1;									// Eintrag inzischen gel鰏cht ?
	if(lRet  != 0)
	...
	if(pData->uSelectedItem && bDo) {							// Ist der ausgew鋒lten Eintrag sichtbar ?
...
//*
//*****************************************************************************
//	Pr黤t ob ein Eintrag sichtbar ist
//	pData		: Zeiger auf die Fensterdaten
//	uItem		: Ist die Nummer des Eintrages
//	uSub		: Untereintrag der gepr黤t werden soll
//						0xFFFFFFFF nur Zeile pr黤en
...
static int TreeListIsVisible(TreeListData *pData, unsigned uItem, unsigned uSub) {
	...
	if(uSub < pData->uColumnCount) {							// Spalte pr黤en
		uNext = pData->aColumn[uSub].bNext;
		iNum  = pData->uSizeX;
		iOff  = pData->uScrollX;
		iAnf  = pData->aColumnXpos[uSub ];
...
//*
//*****************************************************************************
//	L鰏cht einen Eintrag aus dem Fenster
//	pData		: Zeiger auf die Fensterdaten
//	uItem		: Ist die Nummer des Eintrages der gel鰏cht werden soll
//	iMode		: Wie soll der Eintrag gel鰏cht werden
//					0 = Eintrag l鰏chen und nicht neu zeichnen
//					1 = Eintrag l鰏chen und neu zeichnen
//					2 = Nur Kindereintr鋑e l鰏chen und neu zeichnen
//	Ergibt 1 wenn der Eintrag gel鰏cht wurde.
static int TreeListDeleteItem(TreeListData *pData, unsigned uItem, int iMode) {
	...
	if(uItem > pData->uTreeItemsMax) {							// Pr黤e den Eintrag
		if(uItem != U(TVI_ROOT))
			return 0;					// Alles l鰏chen
	...
	if(iMode == 2) {											// Nur Kindereintr鋑e l鰏chen
		if(!pEntry->uFirstChild) {
			return 0;
		}

		while(pEntry->uLastChild) {							// Alle Kinder l鰏chen
	...
	if(uItem == pData->uSelectedItem) {							// Einen ausgew鋒lten Eintrag l鰏chen
		...
		UNLOCK(pData);
		SendNotify(pData, &sNotify.hdr);						// Bekant geben das der Eintrag nicht mehr ausgew鋒lt ist
		LOCK(pData);
	...
	pEntry = pData->pTreeItems[uItem];					// Pr黤en ob der Eintrag noch existiert
	if(!pEntry)
		return 0;

	if(uItem == pData->uTrackedItem) {							// Einen unterstrichenen Eintrag l鰏chen
	...
	for(uPos = 1; uPos < pData->uColumnCount; uPos++) {				// Alle Extraeintr鋑e l鰏chen
	...
	pData->pTreeItems[uItem] = NULL;						// Den Eintrag l鰏chen
	...
	if(pEntry->uState & TVIS_SELECTED)						// Ausgew鋒lte Eintr鋑e runterz鋒len
	...
	iOff  = pData->uScrollY;								// Pr黤e die Scrollposition
	iMax  = pData->uItemPosCount;
	iMax -=	pData->uPageEnties - 1;
...
//*
//*****************************************************************************
//	W鋒lt einen Eintrag ab bzw. an
//	pData		: Zeiger auf die Fensterdaten
//	uItem		: Ist die Nummer des Eintrages der ausgew鋒lt werden soll
//	iMode		: Ist der Grund f黵 die 膎derung
//						TVC_BYKEYBOARD
//						TVC_BYMOUSE
//						TVC_UNKNOWN
//	Ergibt		1 wenn der Eintrag ab/angew鋒lt wurde
//				0 wenn der Eintrag nicht ver鋘dert wurde
...
//*
//*****************************************************************************
//	W鋒lt den Focus-Eintrag
//	pData		: Zeiger auf die Fensterdaten
//	uItem		: Eintrag f黵 den Focus (0xFFFFFFFF=keine 膎derung)
//	uSub		: Spalte f黵 den Focus	(0xFFFFFFFF=keine 膎derung)
...
static int  TreeListSetFocus(TreeListData *pData, unsigned uItem, unsigned uSub) {
	...
	if(!uItem) {												// Focus abw鋒len
		TreeListRemoveFocus(pData);
		return 1;
	}


	if(uItem > pData->uTreeItemsMax) {							// Den Eintrag pr黤en
		return 0;
	}
	...
	if(pTemp) {												// Den alten Eintrag abw鋒len
...
//*
//*****************************************************************************
//	W鋒lt einen Eintrag aus
//	pData		: Zeiger auf die Fensterdaten
//	uItem		: Ist die Nummer des Eintrages der ausgew鋒lt werden soll
//	uSubItem	: Ist die Spalte die gew鋒lt werden soll
//	iMode		: Ist der Grund f黵 die 膎derung
//						TVC_BYKEYBOARD
//						TVC_BYMOUSE
//						TVC_UNKNOWN
//						TVC_ONLYFOCUS	(nur der Focus hat sich ver鋘dert)
//						TVC_DESELECT	(dieses Flag l鰏cht die alte Auswahl)
//						TVC_UNSELECT	(dieses Flag l鰏cht Auswahl bei MultiSel)
//	Ergibt		2 wenn der Eintrag gew鋒lt und umgeklapt wurde
//				1 wenn der Eintrag gew鋒lt wurde
//				0 wenn der Eintrag nicht gew鋒lt wurde
static int TreeListSelectItem(TreeListData *pData, unsigned uItem, unsigned uSubItem, int iMode) {
	...
	if(pData->uStyleEx & TVS_EX_MULTISELECT) {					// Ist die Mehrfachauswahl m鰃lich
		...
		if(!iDel) {
			...
			uOld = 0;
			uSub = 0;
		} else {												// Alle gew鋒lten Eintr鋑e abw鋒len
				...
				for(uPos = pData->uTreeItemsMax; uPos; uPos--) {
					...
					if(TreeListXorSelectItem(pData, uPos, iMode))
						if(!pData->uSelectedCount)
							break;		// Wurden alle Eintr鋑e abgew鋒lt
				}
			}
		}
	} else {													// Altes Select l鰏chen
		iMode &= ~TVC_ONLYFOCUS;
		iDel   =  1;
		iSel   =  0;
	}
	...
	if(SendNotify(pData, &sNotify.hdr)) {						// Abfragen ob der Eintrag gew鋒lt werden darf
		LOCK(pData);
		return 0;
	}
	...
	if(uItem) {												// Pr黤en ob der Eintrag noch existiert
		pEntry = pData->pTreeItems[uItem];
		if(!pEntry)
			return 0;
	}
	...
	if(uItem) {												// Den neuen Eintrag w鋒len
		if(iSel) {
			if(pEntry->uState & TVIS_SELECTED) {
	...
	UNLOCK(pData);
	uRet = U(SendNotify(pData, &sNotify.hdr));					// Anfragen ob die Zweige umgeklappt werden d黵fen
	LOCK(pData);


	pTemp	= pData->pTreeItems[uOld ];						// Zeiger neu holen falls es 膎derungen gab
	pEntry	= pData->pTreeItems[uItem];
...
//	pData		: Zeiger auf die Fensterdaten
//	uItem		: Ist die Nummer des Eintrages der ausgew鋒lt werden soll
//	iMode		: Bit 0 = Untereint鋑e auch 鋘dern
//				  Bit 1 = Eintr鋑e abw鋒len
...
static int TreeListSelectChilds(TreeListData *pData, unsigned uItem, int iMode) {
	...
	for(;;) {
		...
		if(iMode && pEntry->uFirstChild) {					// Auch Kinder 鋘dern
			uItem = pEntry->uFirstChild;
			uLevel++;
			continue;
		}
...
//*
//*****************************************************************************
//	F黦t einen Eintrag ins Fenster ein
//	pData		: Zeiger auf die Fensterdaten
//	pInsert		: Zeiger auf die ein zu f黦enden Daten
//	Ergibt die Einf黦eposition des neuen Eintrages oder 0 bei einem Fehler
static unsigned TreeListInsertItem(TreeListData *pData, TV_INSERTSTRUCT *pInsert) {
	...
	if(uParent > pData->uTreeItemsMax) {						// Pr黤e das Elternelement
	...
	if(pData->uTreeItemsCount + 1 > pData->uTreeItemsMax) {		// Gr鲞e der Liste erh鰄en
	...
	if(uBits & TVIF_CHILDREN) {									// Art der Schaltfl鋍hen
	...
	if(uBits & TVIF_TEXT) {										// Text einf黦en
	...
	if(!pParent) {											// Einen Root-Eintrag einf黦en
		pNew->uParent	= 0;
		uParent			= 0;
		bFlag			= 0;
		uFirst			= 0xFFFFFFFF;
	...
	} else {													// Einen Tree-Eintrag einf黦en
	...
	switch(uAfter) {
		case U(TVI_BEFORE):										// Nach einem Eintrag einf黦en
			if(pParent) {										// Einen Root-Eintrag einf黦en
				pEntry  = pParent;
		...
		case U(TVI_FIRST):										// Am Anfang einf黦en
			if(pFirst[0]) {									// Gibt es schon Eintr鋑e
			...
			pNew ->uNextItem = pFirst[0];					// Eintrag einf黦en
			pFirst[0]		 = uPos;
			break;

		case U(TVI_ROOT):										// Als Root-Eintrag einf黦en
			pNew->uParent	= 0;
			uParent			= 0;
			pFirst			= &pData->uFirstChild;
			pLast			= &pData->uLastChild;

		case U(TVI_LAST):										// Am Ende einf黦en
			if(pLast[0]) {									// Gibt es schon Eintr鋑e
			...
			pNew ->uPrevItem = pLast[0];					// Eintrag einf黦en
			pLast[0]		 = uPos;
			break;


		case U(TVI_SORTEX):										// Einf黦en mittels Funktion
			uItem = pFirst[0];
			if(!uItem) {										// Gibt es keine Kindeintr鋑e
				pFirst[0] = uPos;
				pLast [0] = uPos;
				break;
			}

			if(pNew->bCallback & TVIF_TEXT) {					// Text 黚er Callback holen
				uSize = 1;
				LOCK(pData);
			...
			while(iCount > 0) {											// Binary-Seach Algorithnus
				...
				pEntry  = pItems[uNext];
				if(pEntry->bCallback & TVIF_TEXT) {				// Text 黚er Callback holen
					uSize = 0;
					LOCK(pData);
			...
			if(uBefore) {										// Vorherigen Eintrag anpassen
				...
				pEntry->uNextItem = uPos;
			} else {											// Am Anfang einf黦en
				pFirst[0] = uPos;
			}

			if(uItem) {										// N鋍hsten Eintrag anpassen
				pEntry = pItems[uItem];
				pEntry->uPrevItem = uPos;
			} else {											// Am Ende anh鋘gen
				pLast[0] = uPos;
			}
			break;

		case U(TVI_SORT):										// Alphapetisch einf黦en
			uItem = pFirst[0];
			if(!uItem) {										// Gibt es keine Kindeintr鋑e
				pFirst[0] = uPos;
				pLast [0] = uPos;
		...
		case U(TVI_AFTER):										// Nach einem Eintrag einf黦en
			uAfter  = uParent;
...
DoInsert:
			...
			if(uAfter > pData->uTreeItemsMax) {
				if((uAfter & 0xFFF00000) == 0xFFE00000) {			// In einer genauen Reihe nach Patent einf黦en
					uAfter &= 0xFFFFF;
					...
					if(uAfter == 0) {							// In die erste Reihe einf黦en
						pEntry = pItems[uItem];
						pEntry->uPrevItem = uPos;
						pNew  ->uNextItem = uItem;
						pFirst[0]		  = uPos;
	...
	if(pNew->uState & TVIS_SELECTED) {							// Den ausgew鋒lten Eintrag ausw鋒len
...
//*
//*****************************************************************************
//	膎dert einen Eintrag im Fenster
//	pData		: Zeiger auf die Fensterdaten
//	pItem		: Zeiger auf die ein zu 鋘dernden Daten
...
static int TreeListSetItem(TreeListData *pData, const TV_ITEM *pItem) {
	...
	if(uBits & TVIF_SUBITEM) {									// Einen Extraeintrag 鋘dern
		...
		if(uSub > 0) {
			...
			if(uBits & TVIF_STATE) {							// Den Status 鋘dern
				...
				if((uBits & (TVIS_OVERLAYMASK | TVIS_CUT)) && (pData->hImages || pData->aColumn[uSub].bEdit >= TVAX_CHECK)) {
					uChange = 1;								// Ein Icon hats sich ver鋘dert
				}
				...
				if(uBits & TVIS_SELECTED) {						// Hat sich die Auswahl ge鋘dert
	...
	if(uBits & TVIF_STATE) {
		...
		if(uBits & TVIS_STATEIMAGEMASK) {						// Haben sich die State-Bits ver鋘dert
			if(pData->hStates) {
				uChange = 1;
			}
...
//*
//*****************************************************************************
//	L鰏cht eine Spalte aus dem Header
//	pData		: Zeiger auf die Fensterdaten
//	uCol		: Ist die Nummer der Spalte die gel鰏cht werden soll
//	Ergibt 1 wenn die Spalte gel鰏cht wurde
static int TreeListDeleteColumn(TreeListData *pData, unsigned uCol) {
	...
	for(uPos = 0; uPos < pData->uColumnCount; uPos++) {				// Z鋒lern der variablen Spalten
		if(uPos == uCol)
			continue;
	...
	if(pData->uColumnCount > 0) {								// Liste mit Extraeintr鋑en l鰏chen
		iNum = uCol - 1;
		if(iNum < 0)
			iNum = 0;
		...
		if(pList) {
			for(uItem = 0; uItem <= pData->uTreeItemsMax; uItem++) { // Alle Eintr鋑e aus der Liste l鰏chen
				pExtra = pList[uItem];
				if(!pExtra)
					continue;
	...
	if(!pData->uColumnCount) {								// Den Header l鰏chen
	...
	iXoff = UpdateColumns(pData);							// Hat sich die Spaltenbreiten ver鋘dert
	if(iXoff < 0x10000) {
		sRect.left  = iXoff;
		sRect.left -= pData->uScrollX;
		sRect.top   = pData->uStartPixel;
...
//	pData		: Zeiger auf die Fensterdaten
//	uCol		: Ist die Nummer der Spalte die eingef黦t wird
//	pInsert		: Zeiger auf die ein zu f黦enden Daten
...
static int TreeListInsertColumn(TreeListData *pData, unsigned uCol, TV_COLUMN *pColumn) {
	...
	if(pData->uColumnCount >= MAX_COLUMNS) {					// Pr黤e die Anzahl der Spalten
		return -1;
	}
	...
	if(pColumn->mask & TVCF_TEXT) {								// Auch einen Text 黚ergeben
		sItem.mask    |=  HDI_TEXT;
		sItem.pszText  =  pColumn->pszText;
	}

	if(pColumn->mask & TVCF_MIN) {								// Auch einen Min-Wert 黚ergeben
		iMin		   =  pColumn->iOrder;
		bMinEx		   =  1;
	...
	if(pColumn->mask & TVCF_WIDTH) {							// Fixe Breite f黵 die Spalte
		iWeight		   =  0;
		sItem.mask    |=  HDI_WIDTH;
	...
	if(pData->uColumnCount > 0) {								// Liste mit Extraeintr鋑en erzeugen
	...
	for(uIndex = pData->uColumnCount;;) {						// N鋍hste sichtbare Spalten aktualisieren
		bByte						 = pData->aColumn[uIndex].bIndex;
		pData->aColumn[uIndex].bNext = pData->aColumnPos[bByte + 1];
		if(uIndex == 0)
			break;
	...
	if(pData->uInsertMark) {									// Fehlende Infomarken einf黦en
		TV_ITEM		sSet;
		ExtraItem  *pExtra;
	...
	if(pData->uStyleEx & TVS_EX_HEADERCHGNOTIFY) {				// Ge鋘derte Spalten melden
		sNotify.hdr.code			= TVN_COLUMNCHANGED;
		sNotify.uColumn				= uCol;
		sNotify.uIndex				= pData->aColumn[uCol].bIndex;
		sNotify.uPosX				= pData->aColumnXpos[uCol];
...
//*
//*****************************************************************************
//	Berechnet die Breite der sichtbaren Eintr鋑e einer Spalte
//	pData		: Zeiger auf die Fensterdaten
//	uSub		: Ist die Spalte
...
//*
//*****************************************************************************
//	Pr黤t wo eine Koordinate im Fenster ist
//	pData		: Zeiger auf die Fensterdaten
//	pInfo		: Zeiger auf die ein zu f黦enden Daten
...
static unsigned TreeListHitTest(TreeListData *pData, TV_HITTESTINFO *pInfo) {
	...
	if(iXpos < pData->iIndent) {								// Auf einger點ktem Bereich
...
//*
//*****************************************************************************
//	Setzt das den Einf黦eeintrag
//	pData	: Zeiger auf die Fensterdaten
//	uItem	: Ist die Nummer des Eintrages bei dem eingef黦t werden soll
//	iMode	: 0=davor einf黦en  1=nachher einf黦en
//	Ergibt 1 wenn der Eintrag eingef黦t wurde
...
static COLORREF TreeListSetItemColor(TreeListData *pData, unsigned uItem, unsigned uSub, COLORREF uColor, int iMode) {
	...
	if(uSub >= 255) {											// Die Ganze Zeile 鋘dern
	...
	if(uSub) {												// Extra-Eintrag ver鋘dern
		if(uSub >= pData->uColumnCount)
			return TV_NOCOLOR;
...
static int TreeListSetTrackItem(TreeListData *pData, unsigned uItem, unsigned uSub) {
	...
	if(pData->uTrackedItem) {									// Den alten Eintrag zur點ksetzen
...
static unsigned TreeListFindItem(TreeListData *pData, unsigned uItem, TVFIND *pFind) {
	...
	if(pFind->uFlags & TVIF_CHILD) {						// In den Kindern suchen
		...
		if(uItem > pData->uTreeItemsMax) {					// Ist der Eintrag g黮tig
			return 0;
		}

		if(pFind->uFlags & TVIF_NEXT) {						// Beim n鋍hsten Eintrag weitersuchen
			pEntry = pData->pTreeItems[uItem];
			if(!pEntry)
				return 0;
...
//*
//*****************************************************************************
//	Sucht rekursiv ausgew鋒lte Eintr鋑e
//	pData	: Zeiger auf die Fensterdaten
...
//*
//*****************************************************************************
//	Sucht rekursiv nicht ausgew鋒lte Eintr鋑e
//	pData	: Zeiger auf die Fensterdaten
//	uItem	: Ist die Nummer des Eintrages nach dem mit der Suche begonnen werden soll
//	uStop	: Bestimmt bei welchem Eintrag abgebrochen werden soll werden soll
//	Ergibt die Nummer des Eintrages oder 0 wenn keiner gefunden wurde
...
//	pData	: Zeiger auf die Fensterdaten
//	uItem	: Ist der Eintrag der ge鋘dert werden soll
//	pInfo	:
static void TreeListChangeCheckbox(TreeListData *pData, UINT uItem, int iPosX = 0, int iPosY = 0) {
	...
	if(pData->uStyleEx & TVS_EX_SINGLECHECKBOX) {			// Einzelauswahl
		...
		if(pData->uSingleSel == uItem) {
			...
			if(pData->uSingleSel && pTemp) {				// Anderer Eintrag gew鋒lt
...
//*
//*****************************************************************************
//	Soll ein Mausklick ausgef黨rt werden. ACHTUNG UNLOCK wird ausgef黨rt
//	pData	: Zeiger auf die Fensterdaten
//	uMsg	: Message des Mausklicks
//	wParam	: WPARAM des Mausklicks
//	lParam	: LPARAM des Mausklicks
static void TreeListMouseClick(TreeListData *pData, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	if(uItem) {											// Wurde auf einen Eintrag getr點kt
		pEntry = pData->pTreeItems[uItem];
		...
		if(sInfo.flags & TVHT_ONITEMBUTTON) {				// Eintrag aufklappen
			...
			if(sInfo.flags & TVHT_ONITEMSTATEICON) {		// Checkbox umschalten
				...
				if(sInfo.flags & uMaskItem) {					// Eintrag ausw鋒len
					...
					if(pData->uStyleEx & TVS_EX_MULTISELECT) {		// Mehrfachauswahl erlaubt
						if(uMsg == WM_RBUTTONDOWN)					// Keine Abwahl wenn mehrer Eintr鋑e aus gew鋒lt
						...
						if(uMsg == WM_LBUTTONDOWN)					// Spezialsteuerung f黵 Multiselect bei Darg
						...
						if(wParam & MK_SHIFT) {						// Bis zum angeklicken Ausw鋒len
							if(uMsg != WM_LBUTTONDOWN)
						...
						if(wParam & MK_CONTROL) {					// Einzelen Eintrag umschalten
							...
							if(pEntry && (pEntry->uState & TVIS_SELECTED)) {
								...
								if(TreeListXorSelectItem(pData, uItem, TVC_BYMOUSE)) {
									// Auch unsichtbare Kinder abw鋒len
									pTemp =   pData->pTreeItems[pEntry->uFirstChild];
									if(pTemp && !pTemp->uShowPos)
										for(uTemp = uItem;;) {
					...
					if(wParam & MK_CONTROL) {						// Ist die Ctrl-Taste gedr點kt
						iMode = TVC_BYMOUSE;
					} else {
					...
					if(sInfo.flags & uMaskSub) {					// Extra-Eintrag ausw鋒len
						if(pData->uStyleEx & TVS_EX_SUBSELECT)
							uSub = TVHT_SUBTOCOL(sInfo.flags);
						else
							uSub = 0;
...
//*
//*****************************************************************************
//	Eingabe von Buchstaben 黚er die WM_CHAR Nachricht
//	pData	: Zeiger auf die Fensterdaten
	...
	if(nChar >= ' ') {
		...
		if(!(pData->uStyleEx & TVS_EX_NOCHARSELCET))
			for(;;) {										// Suche Anfangsbuchstaben
				...
				for(i = iNum + 1; i != iNum; i++) {				// Suche 躡ereinstimmung
...
//*
//*****************************************************************************
//	Soll ein Tastentruck ausgef黨rt werden. ACHTUNG UNLOCK wird ausgef黨rt
//	pData	: Zeiger auf die Fensterdaten
//	wParam	: WPARAM des Mausklicks
//	lParam	: LPARAM des Mausklicks
...
static void TreeListKeyDown(TreeListData *pData, WPARAM wParam, LPARAM lParam) {
	...
	if(wParam == VK_RETURN) {								// Wurde Enter gedr點kt
		UNLOCK(pData);
	...
	if(iDel && iScr)										// Ende oder Anfang w鋒len
	...
	if(iDel && iScr) {									// Das Fenster scrollen
		...
		switch(wParam) {
			...
			case VK_SPACE:
				if(pEntry == NULL)
					break;				// Expantieren und schlie遝n
				if(pEntry->bFlags & TVIX_HASBUTTON) {
	...
	} else {													// Einen anderen Eintrag ausw鋒len
		iSub  = pData->uSelectedSub;
		...
		if(pData->uItemPosCount > 0)
			if(iLine != iOldLine || iCol != iOldCol) {			// Wurde ein anderer Eintrag ausgew鋒lt
				if(pData->uSelectedCount > 1) {
					TreeListRemoveFocus(pData);
				} else {
					pData->uSelectedBase = uTemp;
				...
				if(pData->cReSelect && pData->uSelectedBase) {	// Shift-Select neu ausw鋒len
					uTemp = pData->uSelectedBase;
					pTemp = pData->pTreeItems[uTemp];
					uStop = uItem;
				...
				if((pData->uStyleEx & TVS_EX_MULTISELECT) && !iDel) {
					...
					for(; iPos >= iMax; iPos--) {					// 躡ersprungene Eintr鋑e mit aus/abw鋒len
						...
						pTemp = pData->pTreeItems[uItem];	// Auch unsichtbare Kinder abw鋒len
						if(pTemp) {
...
//*
//*****************************************************************************
//	Sortiert die Kindereintr鋑e eines Eintrages mitteles einer Funktion
//	pData		: Zeiger auf die Fensterdaten
...
static int TreeListSortItemsEx(TreeListData *pData, TV_SORTEX *pSortData, int iMode) {
	...
	if(iMode) {											// Sortiere die Untereintr鋑e
		pNext				= pList[uFirst];
		sSort.hParent		= (HTREEITEM)uFirst;
		sSort.lParam		= pSortData->lParam;
		sSort.lpfnCompare	= pSortData->lpfnCompare;
...
//******************** Eintr鋑e neu einsortirenen *****************************
	...
	for(uNum = 0; uNum < uPos;) {								// Kinder neu einh鋘gen
		pEntry = pList[uItem];
...
//*
//*****************************************************************************
//	Sortiert die Kindereintr鋑e eines Eintrages via Text
//	pData		: Zeiger auf die Fensterdaten
//	pSortData	: Ist ein Zeiger auf die Sortiertaten
//	iMode		: 1=Rekursiv sortieren
//	Ergibt 1 wenn ok sonst 0
...
static int TreeListEndLabelEdit(TreeListData *pData, int iMode) {
	...
	if(uSub) {											// Wurde der Text in einer Sub-Spalte ge鋘dert
		pExtra = pData->pExtraItems[uSub - 1][uItem];
		if(!pExtra) {
	...
	if(cCb) {												// Callback aufrufen
		...
		if(pData->aColumn[uSub].bFlags & TVAE_NEXTLINE &&			// In die n鋍hste Zeile springen
		        iMode == 2 && iAuto != TVAX_NONE && iAuto != TVAX_STEP) {
			if(!pData->cColumnStart) {
				if(uSub) {									// Hat sich der Text ver鋘dert ?
					pExtra = pData->pExtraItems[uSub - 1][uItem];
...
static HWND TreeListEditLabel(TreeListData *pData, unsigned uItem, unsigned uSub) {
	...
	if(pData->hEdit) {									// Editfenster l鰏chen
...
static int TreeListStartNotifyEdit(TreeListData *pData, unsigned uItem, unsigned uSub, WPARAM wParam, LPARAM lParam) {
	...
	if(wParam >= ' ' && wParam <= 0xFFFF) {					// Shift und Cltr pr黤en
	...
	if(lRet & TVIR_EDITCOMBOBOX) {							// Die Combobox f黮len
		uMax = sNotify.uMaxEntries;
		iSel = -1;

		if(sNotify.pTextList) {							// Texte 黚er Listenfeld
			...
			for(uCnt = 0; uCnt < uMax; uCnt++) {
				for(uLen = 0; pText[uLen]; uLen++) {			// Subtextl鋘ge holen
					if(pText[uLen] == cChar)
						break;
				}
			...
			if(lRet & TVIR_EDITCOMBODEL) {					// Den Puffer l鰏chen
				delete(TCHAR *)sNotify.pTextEntries;
			}
		}

		if(iSel >= 0) {										// Listeneintrag ausw鋒len
			SendMessage(hWnd, CB_SETCURSEL, iSel, 0);
		}

		if(sNotify.uHeight) {								// H鰄e der Dropdown-Liste einstellen
...
//*
//*****************************************************************************
//	Startet die Autoeditierung f黵 eine Spalte
//	pData		: Zeiger auf die Fensterdaten
...
//******************** Weiterschalten mit Return ******************************
	if(uMode >= TVAX_STEP) {
		...
		if((uMode & 1) && wParam != VK_ICONCLK) {					// Ein Edit-Feld 鰂fnen
			uMode = TVAX_EDIT;
			goto EditField;
		}
		...
		if(uBits & TVAE_PTRLIST) {								// Zeigerliste  char *pTexte[]={"1","2",NULL};
			...
			for(uPos = 0;; uPos++) {
				...
				for(uLen = 0; pText[uLen]; uLen++) {				// Subtextl鋘ge bestimmen
					if(pText[uLen] == cChar)
						break;
				}
			...
			for(uLen = 0; pText[uLen] && uLen < 256; uLen++) {		// Ausgew鋒lten Text kopiren
				if(pText[uLen] == cChar)
					break;
...
EditField:
	...
	// Ersten Buchstaben ans Fenster 黚ergeben
...
//*
//*****************************************************************************
//	Ist die Fensterfunktion f黵 das TreeList Fenster
static LRESULT CALLBACK TreeListProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	switch(uMsg) {
		...
		case WM_DESTROY:											// Das Fenster zerst鰎en
		...
		case WM_NCDESTROY:										// Das Fenster zerst鰎en
		...
		case WM_SIZE:												// Die Fenstergr鲞e wurde ver鋘dert
			...
			if(uVal && uVal != pData->uSizeX) {
				...
				if(pData->uColumnCountVar) {					// Spalten mit variabler Breite nach f黨ren
					RECT	sRect;
					int		iDelta;
					int		iNum;
				...
				if(uVal > uOld) {								// Hat sich die Breite vergr鲞ert
					RECT	sRect;
			...
			if(uVal && uVal != pData->uSizeY) {
				if(uVal > pData->uSizeY) {						// Hat sich die H鰄e vergr鲞ert
					RECT	sRect;
				...
				pData->uMaxEnties   /= pData->iRowHeight;
				pData->uPageEnties  /= pData->iRowHeight;
				// Wenn H鰄e vergr鲞ert dann Scroll-Position pr黤en
		...
		case WM_LBUTTONDOWN:										// Mausklick
			...
			if(pData->cButtonFlag && pData->uToolTipItem) {	// Doppelklick simulieren 黚er ToolTip
				pData->cButtonFlag = 0;
		...
		case WM_SYSCOLORCHANGE:										// Wurden die Systemfarben ver鋘dert
		...
		case WM_STYLECHANGED:										// Hat sich der Fenstersytle ge鋘dert
		...
		case WM_TIMER:												// Timer Funktion
			...
			if(wParam == ID_TOOLTIPCHECK) {
				pData	= GetHandle(hWnd);
				if(pData->uToolTipItem) {
					if(pData->uToolTipShow) {					// Verz鰃ertes einblenden
						pData->uToolTipShow--;
		...
		case WM_NOTIFY:												// Notify Nachrichnten
			...
			if(wParam == 1) {									// Nachricht vom Header
				...
				iCode = pHdr->hdr.code;
				// Hat sich die Spaltenbreite ver鋘dert
				...
				if(iCode == HDN_BEGINTRACK) {					// User will die Spaltenbreite 鋘dern
					pData	= GetHandle(hWnd);
					...
					if(pData->uStyleEx & TVS_EX_NOCOLUMNRESIZE) {	// Darf der User die Spaltenbreite 鋘dern
						return 1;
					}
					...
					if(pData->uStyleEx & TVS_EX_FIXEDCOLSIZE) {	// Fixe gesammte Spaltenbreite
						...
						for(iNext++;; iNext++) {				// Suche n鋍hste ver鋘erbare Spalte
					...
					if(U(pHdr->iItem) < pData->uColumnCount)		// Darf die Spalte ver鋘dert werden
						if(pData->aColumn[pHdr->iItem].sFixed) {
							...
							GetCursorPos(&sPoint);					// Die n鋍hste ver鋘derbare Spalte greifen
				...
				if(iCode == HDN_DIVIDERDBLCLICK) {				// Doppelcklick auf Spaltentrenner
					...
					if(iSize) {								// Spalte auf maximale Textbreite
						...
						if(pData->aColumn[pHdr->iItem].bMinEx)	// Minimale Breite pr黤en
				...
				if(iCode == HDN_TRACK || iCode == HDN_ENDTRACK) {	// Wurde die Spaltenbreite ver鋘dert
					pData	= GetHandle(hWnd);
					...
					if(pData->aColumn[iCol].bMinEx)				// Minimale Breite pr黤en
					...
					if(pData->uStyleEx & TVS_EX_FIXEDCOLSIZE) {	// Fixe gesammte Spaltenbreite
						...
						for(iNext++;; iNext++) {				// 躡erspringe fixierte Spalten
							if(U(iNext) >= pData->uColumnCount) {
								UNLOCK(pData);
								return 0;
							}
						...
						if(iDelta < 0) {						// N鋍hste Spalte wird zu klein
							if(sTemp.cxy < 0) {
								sTemp.cxy  =  0;
							...
							if(pData->aColumn[iSub].bMinEx)		// Minimale Breite pr黤en
		...
		case TVM_GETSETOPTION:										// Diverse Optionen einstellen
			...
			switch(U(wParam) & ~TVOP_WRITEOPTION) {
				case TVOP_AUTOEXPANDOFF:							// Icon Offset f黵 TVS_EX_AUTOEXPANDICON
		...
		case TVM_GETITEM:											// Eintr鋑e abfragen
		...
		case TVM_SETITEM:											// Eintr鋑e einf黦en
		...
		case TVM_INSERTITEM:										// Eintr鋑e einf黦en
		...
		case TVM_DELETEITEM:										// Eintr鋑e l鰏chen
		...
		case TVM_FINDITEM:											// Eintr鋑e suchen
		...
		case TVM_DELETECOLUMN:										// L鰏chen einer Salte im Header
		...
		case TVM_INSERTCOLUMN:										// Einf黦en einer Salte im Header
...
ColSet:
		...
		case TVM_SELECTCHILDS:										// Mehrere Eintr鋑e ausw鋒len
		...
		case TVM_SELECTSUBITEM:										// Einen (Sub)Eintrag ausw鋒len
		...
		case TVM_SELECTDROP:										// Den unterstrichenen Eintrags ausw鋒len
		...
		case TVM_SELECTITEM:										// Einen Eintrag ausw鋒len
		...
		case TVM_GETCOUNT:											// Anzahl der Eintr鋑e abfragen

			pData = GetHandle(hWnd);
			return pData->uTreeItemsCount;

		case TVM_GETINDENT:											// Einr點kung abfragen

			pData = GetHandle(hWnd);
			return pData->iIndent;

		case TVM_SETINDENT:											// Einr點kung einstellen
		...
		case TVM_GETITEMHEIGHT:										// Zeilenh鰄e abfragen

			pData = GetHandle(hWnd);
			return pData->iRowHeight;

		case TVM_SETITEMHEIGHT:										// Zeilenh鰄e abfragen
...
NextExp:
		...
		case TVM_SETINSERTMARK:										// Einf黦emarke eintellen
		...
		case TVM_SETITEMBKCOLOR:									// Hintergrundfarbe eines Eintrages 鋘dern
		...
		case TVM_SETITEMTEXTCOLOR:									// Textfarbe eines Eintrages 鋘dern
		...
		case TVM_SORTCHILDREN:										// Sortieren der Kindereintr鋑e
		...
		case TVM_SETTOOLTIPS:										// Das Handle f黵 das ToolTip-Fensters setzen
		...
		case TVM_SETUSERDATASIZE:									// Einstellen der Gr鲞e der User-Daten
		...
		case TVM_GETUSERDATASIZE:									// Abfragen der Gr鲞e der User-Daten
		...
		case TVM_SETCOLUMN:											// Einen Spalten-Header einstellen
			...
			if(pData->hHeader && U(wParam) < pData->uColumnCount) {
				...
				if(lRet && (pCol->mask & TVCF_FMT)) {			// Hat sich die Ausrichtung ver鋘dert
					BYTE bAlign;
		...
		case TVM_COLUMNAUTOEDIT:									// AutoEdit f黵 eine Spalte einstellen
		...
		case TVM_COLUMNAUTOICON:									// Icons f黵 AutoEdit einstellen
...
//******************** Eintr鋑e zeichnen **************************************
	for(; uPos < uMax; uPos++) {									// Alle Eintr鋑e ausgeben
		uItem = pData->pItemPos[uPos];
		...
		if((pEntry->uState & TVIS_SELECTED) && (uStyleEx & TVS_EX_FULLROWMARK)) {
			...
			if(uPos & 1) {										// Farbe wechselweise 鋘dern
		...
		if(uStyle & TVS_HASLINES) {
			...
			for(i = iLevel; i > 0; i--) {							// Bereich vor Schaltfl鋍hen
		...
		if(uStyle & TVS_HASBUTTONS) {							// Fenster mit Schaltfl鋍hen ?
			...
			if(uStyle & TVS_HASLINES) {							// Linien unter Schaltfl鋍hen
				MoveToEx(hDc, iXpos, sArea.top | 1, NULL);
			...
			if(pEntry->bFlags & TVIX_HASBUTTON) {				// Schaltfl鋍hen zeichnen
			...
			if(uStyle & TVS_HASLINES) {						// Nur Linien zeichnen ohne Schaltfl鋍hen
				sArea.right = sArea.left + iIndent;
				ExtTextOut(hDc, 0, 0, ETO_OPAQUE, &sArea, NULL, 0, NULL);
...
NoRootLines:
		...
		if(uBits & (TVIS_SELECTED | TVIS_DROPHILITED | TVIS_UNDERLINE | TVIS_TRACKED | TVIS_TEXTCOLOR | TVIS_FOCUSED)) {
			...
			if((uBits & TVIS_SELECTED) && pData->cHasFocus && uItem == pData->uSelectedItem && !pData->uSelectedSub) {
				...
				if(uBits & TVIS_DROPHILITED) {
					...
					if(uBits & TVIS_SELECTED) {				// Ist das Feld ohne Focus ausgew鋒lt
						if(pData->cHasFocus) {
							uTempColor =     GetSysColor(COLOR_HIGHLIGHTTEXT);
							SelectObject(hDc, GetSysColorBrush(COLOR_HIGHLIGHT));
							SelectObject(hDc, GetStockObject(NULL_PEN));
			...
			// Ist der Text gr鲞er als die Spalte
		...
		if(uStyleEx & TVS_EX_ITEMLINES) {						// Linien um den Eintrag zeichnen
			...
			if(iLast > iStart) {								// Ist das Feld nach links einger點kt gegen黚er dem oberen
				sArea.top--;
		...
		//************ Extraeintr鋑e zeichnen *********************************

ExtraDraw:

		uNextColor		 = uOutColor;							// Hintergrundfarbe f黵 die n鋍hste Spalte
		...
		for(uColumn = 1; uColumn <= pData->uColumnCount; uColumn++) {	// Extra Spalten zeichnen
			...
			if(!pExtra) {										// Leeres Feld zeichnen
				...
				if(pExtra->bCallback) {						// Text 黚er Callback holen
				...
				if(uBits & (TVIS_SELECTED | TVIS_DROPHILITED | TVIS_UNDERLINE | TVIS_TRACKED | TVIS_TEXTCOLOR | TVIS_FOCUSED)) {
					...
					// Ist das Feld ohne Focus ausgew鋒lt
	...
	if(sArea.top < sRect.bottom) {								// Untere Fl鋍he ohne Eintr鋑e f黮len
